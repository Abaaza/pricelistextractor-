...
class DrainageExtractor:
    ...
    def is_header_row(self, row: int) -> Optional[str]:
        ...
        # A header row has no value in column A but has text in column B
        col_a = self.worksheet.cell(row=row, column=1).value
        col_b = self.worksheet.cell(row=row, column=2).value
        
        # Header criteria: Column A is empty/None and Column B has substantial text
        if col_a is None and col_b and isinstance(col_b, str) and len(col_b) > 10:
            return col_b
        return None
    ...
    def extract_items(self):
        ...
        for row in range(1, self.worksheet.max_row + 1):
            ...
            # Check if this row has an item code
            ...
            if range_info:
                # Row has range data - combine with header
                range_desc, unit, separator = range_info
                full_description = f"{current_header}; depth to invert: {range_desc} {unit}"
            else:
                # Row doesn't have range data - check for description in column B or C
                col_b = self.worksheet.cell(row=row, column=2).value
                col_c = self.worksheet.cell(row=row, column=3).value
                
                if col_b and isinstance(col_b, str):
                    # Use column B as the description
                    full_description = col_b
                    unit = self.worksheet.cell(row=row, column=6).value or 'item'
                elif col_c and isinstance(col_c, str):
                    # Use column C as additional info (e.g., pipe sizes)
                    if current_header:
                        full_description = f"{current_header}; {col_c}"
                    else:
                        full_description = col_c
                    unit = self.worksheet.cell(row=row, column=6).value or 'm'
                else:
