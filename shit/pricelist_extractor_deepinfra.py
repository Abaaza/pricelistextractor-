"""
MJD Pricelist Extraction with DeepInfra Qwen 72B Enhancement
Uses DeepInfra's Qwen2.5-72B-Instruct model for cost-effective enhancement
"""
...
# OpenAI library will work with DeepInfra
try:
    import openai
...
class DeepInfraEnhancer:
    """Handles DeepInfra Qwen API calls for data quality improvement"""
    ...
    def __init__(self, api_key: str = DEEPINFRA_API_KEY):
        ...
        if api_key and OPENAI_AVAILABLE:
            ...
            print("DeepInfra Qwen 72B client initialized successfully")
            print(f"  Model: {DEEPINFRA_MODEL}")
            print(f"  Endpoint: {DEEPINFRA_BASE_URL}")
        else:
            self.client = None
            if not OPENAI_AVAILABLE:
                print("Warning: OpenAI library not installed - enhancement disabled")
            else:
                print("Warning: No DeepInfra API key - enhancement disabled")
        self.cache = {}
        self.total_tokens = 0
        self.api_calls = 0
    
    def enhance_batch_items(self, items: List[Dict], sheet_name: str) -> List[Dict]:
        """Enhance a batch of items using DeepInfra Qwen 72B"""
        if not self.client:
            return items
        
        # Process in batches of 10 for optimal performance
        # Qwen 72B can handle larger contexts efficiently
        batch_size = 10
        enhanced_items = []
        ...
        for i in range(0, len(items), batch_size):
            ...
            print(f"    Processing batch {batch_num}/{total_batches} with Qwen 72B...")
            ...
            try:
                ...
                print(f"      Warning: Batch {batch_num} failed - {str(e)[:50]}")
                enhanced_items.extend(batch)  # Use original if enhancement fails
            
            # Rate limiting - DeepInfra has generous limits but let's be safe
    ...
    def _process_batch(self, batch: List[Dict], sheet_name: str) -> List[Dict]:
        """Process a batch of items with DeepInfra Qwen"""
        try:
            # Prepare batch data
            ...
            prompt = f"""You are an expert construction cost estimator reviewing pricelist items from Excel.
...
1. **Description**: 
   ...
   - Keep product codes/standards (BS, EN, ISO, etc.)
   - Make professional and clear
   - DO NOT fundamentally change the meaning

2. **Unit**: Standardize to construction units:
   - Length: m, lm
   - Area: m², m2
   - Volume: m³, m3
   - Weight: kg, tonnes, t
...
5. **Keywords**: Generate 4-6 construction-specific search keywords
...
[{{
  ...
  "keywords": ["keyword1", "keyword2", "keyword3", "keyword4"]
            ...
            # Make API call to DeepInfra Qwen
            response = self.client.chat.completions.create(
                model=DEEPINFRA_MODEL,
                messages=[
                    {"role": "system", "content": "You are an expert construction estimator. Return only valid JSON array."},
            ...
            try:
                ...
                if not isinstance(enhanced_data, list):
                    print(f"      Warning: Response not a list, using original data")
                    return batch
            ...
            for item in batch:
                ...
                # Find matching enhancement by index
            ...
            print(f"      Error in DeepInfra enhancement: {str(e)[:100]}")
            return batch

class SheetExtractor:
    """Base class for sheet-specific extraction logic"""
    
    def __init__(self, sheet_name: str, worksheet, enhancer: Optional[DeepInfraEnhancer] = None):
    ...
    def is_valid_rate(self, value: Any) -> bool:
        ...
        if value is None:
            return False
        if isinstance(value, str):
            skip_values = ['by others', 'included', 'n/a', 'nil', '-', 'tbc', 'tbd', 'item']
    ...
    def post_process_items(self, items: List[PriceItem]) -> List[PriceItem]:
        """Post-process items with DeepInfra enhancement"""
        if not self.enhancer or not items:
            return items
        
        print(f"  Enhancing {len(items)} items with DeepInfra Qwen 72B...")
...
class DeepInfraPricelistExtractor:
    """Main extractor with DeepInfra Qwen enhancement"""
    
    SHEET_EXTRACTORS = {
        'Groundworks': GroundworksExtractor,
        'RC works': GroundworksExtractor,  # Use same logic
        'Drainage': GroundworksExtractor,
        # All others use GenericExtractor
    }
    
    SKIP_SHEETS = ['Summary', 'Set factors & prices', 'Tender Summary', 'Budget Costings']
    
    def __init__(self, file_path: str, use_deepinfra: bool = True):
        self.file_path = file_path
        self.use_deepinfra = use_deepinfra
        self.workbook = None
        self.all_items = []
        
        # Initialize DeepInfra enhancer
        if use_deepinfra:
            self.enhancer = DeepInfraEnhancer(DEEPINFRA_API_KEY)
        else:
            self.enhancer = None
    ...
    def extract_all_sheets(self) -> List[PriceItem]:
        ...
        for sheet_name in self.workbook.sheetnames:
            ...
            if items:
                ...
                print(f"  Extracted and enhanced {len(items)} items")
            else:
    ...
    def print_extraction_summary(self):
        ...
        if self.enhancer and hasattr(self.enhancer, 'api_calls'):
            print(f"\nDeepInfra API usage:")
            print(f"  Total API calls: {self.enhancer.api_calls}")
            print(f"  Model used: {DEEPINFRA_MODEL}")
    
    def export_to_csv(self, output_file: str = "pricelist_deepinfra.csv"):
    ...
    def export_to_json(self, output_file: str = "pricelist_deepinfra.json"):
...
def main():
    ...
    print("="*60)
    print("MJD PRICELIST EXTRACTION - DEEPINFRA QWEN 72B")
    print("="*60)
    print(f"Input file: {file_path}")
    print(f"AI Enhancement: DeepInfra Qwen2.5-72B-Instruct")
    print(f"Cost-effective: ~90% cheaper than GPT-4")
    print(f"Schema: Updated database structure")
    
    # Check if OpenAI library is available
    if not OPENAI_AVAILABLE:
        print("\nWarning: OpenAI library not installed!")
        print("Run: pip install openai")
        response = input("\nContinue without AI enhancement? (y/n): ")
        if response.lower() != 'y':
            return
        use_deepinfra = False
    else:
    ...
    # Create extractor
    extractor = DeepInfraPricelistExtractor(file_path, use_deepinfra=use_deepinfra)
    
    # Extract all sheets
    print("\nStarting extraction with DeepInfra enhancement...")
    ...
    extractor.export_to_csv("pricelist_deepinfra.csv")
    extractor.export_to_json("pricelist_deepinfra.json")
    
    print("\n" + "="*60)
    print("EXTRACTION COMPLETE WITH DEEPINFRA!")
    print("="*60)
    print("\nOutput files:")
    print("  pricelist_deepinfra.csv - For spreadsheet import")
    print("  pricelist_deepinfra.json - For database import")
